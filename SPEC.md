...
47. **The build dogfoods: vulnix scans the project's own flake source closure (`${self}`)** purely to trigger feed retrieval into the cache-dir (§V.45). Any in-sandbox store path would do; scanning `self` means the mirror is exercised against its own closure — a self-test — and keeps the trigger deterministic and input-pinned (no external `pkgs.hello`-style dependency needed beyond what's already in scope).
...
in pkgs.stdenv.mkDerivation {
  ...
  buildPhase = ''
    ...
    # dogfood: scan our own flake source closure to drive feed retrieval (§V.47)
    vulnix -m http://127.0.0.1:8731/ -c $TMPDIR/cache ${self} || true   # vuln-exit ≠ build fail (§V.45)
