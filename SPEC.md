...
11. **2026-08-17 — `nix flake check` failed while evaluating the actionlint check because the referenced set-and-setting helper passed a scalar regex to Nixpkgs' `sourceByRegex`, whose current API requires a list.** Fixed by retaining the actionlint check as a consumer-owned extra check and passing the list-shaped API explicitly.
