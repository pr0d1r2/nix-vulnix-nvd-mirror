...
42. **`.gitignore` must always ignore `public/` and `staging_dir`, and must never ignore `feeds.lock`.** `sync-setting` composes the repo `.gitignore` from fragments (§V.20); the composition MUST include a project fragment ignoring `public/` and the staging dir (its default basename `nvd-part/`, for a `STAGING_DIR`/`TMPDIR` pointed inside the checkout), or a sync would un-ignore the generated feeds and let `download.sh`'s output get committed — breaking the gitignored-`public/` premise the whole FOD/`feeds.lock` design rests on (§V.15). `feeds.lock` is committed (§V.26) and must stay tracked. `test_flake.sh` verifies it with pure file checks (the flake-check sandbox has no `.git`).
...
- **Checkout:** `fetch-depth: 0` so the daily `feeds.lock` commit can `git pull --rebase` onto current `main` before it is proposed as a PR (step 8).
...
- **Steps (deploy job, after gate):**
  ...
  5. Regenerate `feeds.lock` (`nix hash file --sri --type sha256` per `public/*.json.gz`) in the working tree — `git pull --rebase` first so the later commit sits on current `main`. It is **not** committed here: the build below reads it, and its landing on `main` is the last step (§B.27).
  ...
  8. **Publish feed lock** — `main` is protected (a PR is required), so a changed `feeds.lock` is committed on `bot/feeds-lock`, force-pushed, opened as a PR and merged by the job itself (`gh pr merge --squash --delete-branch`, `GH_TOKEN` = `github.token`, `pull-requests: write`). Runs last, so a refused lock can never skip steps 6–7 (§B.27). Requires the repository setting *Allow GitHub Actions to create and approve pull requests*.
...
25. **2026-09-25 — the tending loop's T17 build made its own pushes unpushable.** T17 was still open although the devShell already existed via `set-and-setting.lib.mkDevShells` (#31). Re-implementing it replaced that shell with a hand-rolled `mkShell` that dropped `materialization.packages`, so `lefthook-markdownlint`, `lefthook-markdownlint-agentic` and `lefthook-linter-coverage-full` vanished (a §B.23 recurrence, exit 127), and its new `.gitignore` test ran `git ls-files` inside the flake-check sandbox, which has no `.git`, so it could never pass. Fixed by closing T17 on the existing shell: the missing §V.19 tools join `basePackages`, `.gitignore` ignores the staging basename `nvd-part/`, and `test_flake.sh` guards the toolchain, the `materialization.packages` base, the `.gitignore` rules (file checks only) and `.envrc`.
26. **2026-09-25 — pre-push stayed red on `main` even with every wrapper on PATH.** Three hooks failed on the untouched tree: `actionlint` (the `lefthook-actionlint` wrapper execs `actionlint` from PATH, which the devShell never provided), `markdownlint` (bare fences in `README.md`/`CONTRIBUTING.md`, MD040/MD031), and `linter-coverage`. The last had two causes: nothing set `LEFTHOOK_LINTER_COVERAGE_DOC` for this repo (a caller's leaked value pointed at a vendored-era `agent/set/skills/linter.md` that no longer exists), and the pinned `set-and-setting` builds `lefthook-linter-coverage-full` with its generic `wrap`, which never substitutes the `LEFTHOOK_LINTER_COVERAGE_AWK_PROGRAM_PATH` placeholder, so gawk cannot open its program. Fixed locally: `pkgs.actionlint` joins the devShell, fences carry languages, `docs/linters.md` holds the extension table and the `settingHook` exports its path, and the flake rebuilds the coverage wrapper from `set-and-setting.inputs.nix-lefthook-linter-coverage-src` with the placeholder substituted, replacing the broken one. Drop the rebuild once `set-and-setting` substitutes it itself.
27. **2026-09-26 — every scheduled mirror run since 2026-09-21 failed after deploying, and the refreshed cache was never built.** `main` became protected (a PR required, 0 approvals, no required checks, `enforce_admins`), and the "Update feed lock" step still ran `git push origin HEAD:main`: `GH006: Protected branch update failed`. Pages deployed, but the lock never reached `main` (the drift #35/#36/#39 fixed by hand), and because the push step failed, the pre-seed, `nix build .#nvd-cache` and Cachix push after it were skipped — consumers got no fresh pre-built cache either. Fixed without loosening protection: the lock is regenerated before the build as before, and a new last step commits it to `bot/feeds-lock`, opens a PR and merges it itself (step 8). Needs the repository setting *Allow GitHub Actions to create and approve pull requests* (`can_approve_pull_request_reviews`), an Actions setting, not branch protection.
