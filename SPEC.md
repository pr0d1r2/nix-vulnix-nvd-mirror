...
| `.` | T25 | **Resumable per-page fetch** (foundation for T28): `_fetch_page` with per-page exponential backoff + jitter and `Retry-After` handling, `${staging_dir}/<feed>/` checkpoint **outside `public/`**, resume-from-`lastIndex`, `totalResults` completeness guard, atomic gzip assembly (§V.28–§V.32) — makes the keyless path reliable |
