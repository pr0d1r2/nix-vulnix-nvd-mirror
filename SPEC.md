...
21. `flake.nix` exposes `checks.${system}` that wrap every current verification script — `shellcheck` lint, `test_download.sh`, `test_checksum.sh`, `test_flake.sh`, `test_health_check.sh` — so `nix flake check` is the single command that runs all checks. CI must invoke `nix flake check` rather than calling the scripts ad-hoc. **CI builds first, then checks:** because `nix flake check` would otherwise build the flake's derivations itself (including `packages.nvd-cache`), CI explicitly `nix build`s the intended closures (checks + devShell) — pushing them to Cachix — and then runs **`nix flake check --no-build`**, which only evaluates + runs the already-built checks and **builds nothing**. This keeps `nvd-cache` out of CI (it is mirror-only, §V.22) and keeps the cache-push and the validation as distinct, ordered steps.
...
48. **The build's HTTP server synthesizes an empty feed for any absent in-range year (404-abort immunity).** vulnix computes its requested year set as `current-5…current` from **today's date** and calls `raise_for_status()` — so a single missing year feed (HTTP 404) **aborts the whole update**. But `feeds.lock`/`${feedFarm}` is a *snapshot* from the last mirror run, which can lag vulnix's date at year rollover (e.g. a build on Jan 1 wants the new year before the mirror has published it). To stay robust, the server does **not** plain-serve a static dir: for a requested `nvdcve-2.0-<year>.json.gz` that is **not** in `feedFarm`, it returns a synthetic **empty-but-valid** feed (`{…,totalResults:0,vulnerabilities:[]}`, gzipped) with HTTP 200 instead of 404; real feeds are served as-is. This makes the build immune to the build-date↔lock-snapshot skew (and to vulnix requesting a year the mirror legitimately has no CVEs for). Consumers using the **raw mirror** (`vulnix --mirror <pages>`) instead of the prebuilt cache face the same rollover skew for ~one mirror cycle (§V.38 publishes empty current-year feeds to minimise it); the prebuilt `nvd-cache` substitute is unaffected.
47. **The build scans a minimal real package (`pkgs.hello`) to trigger retrieval.** vulnix processes the scan target *before* NVD retrieval: `populate_store()` → `store.add_path(target)` parses derivations, **then** the feeds are fetched. So the target must be a **real built store path** (with deriver info), not a source dir — `${self}` (the flake source) has no deriver and would risk `add_path` aborting before retrieval ever runs. `pkgs.hello` is a tiny, fully-built derivation with essentially **no CVE history** (clean scan output; its exit is decoupled anyway, §V.45) and a minimal closure, so it reliably drives feed retrieval into the cache-dir. Any small built package would serve; `hello` is the convention.
...
- **Steps (build first, then check — §V.21):**
  ...
  3. **Build** the intended closures explicitly: `nix build .#devShells.${system}.default .#checks.${system}.*` — these get pushed to Cachix by step 2.
  4. **Check** with `nix flake check --no-build` — evaluates + runs the (already-built) `checks.${system}`; **builds nothing** (§V.21, §V.34).
- **Never builds `nvd-cache`** — it is not in the step-3 build set and `--no-build` won't build it; the cache build is mirror-only (§V.22).
...
| `.` | T20 | Add `.github/workflows/ci.yml` (push/PR/`workflow_call`): install Nix, **build** `.#devShells.${system}.default .#checks.${system}.*` (pushed to `pr0d1r2.cachix.org` via `cachix-action` + `CACHIX_AUTH_TOKEN`, skip when absent), **then** `nix flake check --no-build`. Never builds `nvd-cache` (§V.21, §V.22, §V.23) |
...
| `.` | T29 | **Verify vulnix consumes the published 2.0 feed layout** (§B.6, §V.40): drive the pinned `vulnix -m http://127.0.0.1:<port>/ -c <dir> ${pkgs.hello}` (loopback-served feeds, §V.46; minimal-package scan, §V.47; exit decoupled, §V.45); confirm `nvd-cache` builds a non-empty `Data.fs`. (vulnix source confirms 2.0 + our filenames; this closes the residual end-to-end check.) |
