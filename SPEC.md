...
11. **2026-07-22 — guardrail coherence ran outside the declared development shell.** The reusable CI workflow materialized configuration with `nix develop --command true`, then launched `nix run .#confirm` in a fresh environment. The generated Markdown and YAML hooks were valid, but their three fragment-provided wrapper executables were no longer on `PATH`, causing false coherence failures. Fixed by running materialization and confirmation together through `nix develop --command`, while retaining the full flake checks on Linux and macOS.
12. **2026-07-22 — executable modes violated the generated repository policy.** Five shell test/publishing helpers were tracked with execute permission even though the guardrail permits only explicitly designated executables; `execute-permissions-check` therefore rejected the tree. Fixed by tracking those helpers as regular files (`0644`), consistent with their explicit `bash` invocation sites.
13. **2026-07-22 — generated guardrail policy files were absent and the shell policy was over-broad.** The flake enabled editorconfig and file-size checks without committing their required `.editorconfig` and `config/lefthook/file_size_limits.yml` inputs, so every tracked file had an empty expected charset and the size check had no policy to load. It also selected the functionless-shell architecture fragment for an intentionally function-based shell application. Fixed by committing the generated policies, formatting the tracked sources, and retaining the applicable pinned `shellcheck` and `shfmt` checks explicitly without the inapplicable architecture constraint.
