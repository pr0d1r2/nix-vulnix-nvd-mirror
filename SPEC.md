...
46. **Feeds are served to vulnix over loopback HTTP at build time (vulnix is HTTP-only).** vulnix fetches via `requests.get(mirror + file)` and ships **no `file://` adapter**, so `-m file://…` raises `InvalidSchema`. The `nvd-cache` build instead starts a throwaway `python -m http.server` bound to `127.0.0.1` serving `${feedFarm}`, and points `vulnix -m http://127.0.0.1:<port>/`. The Nix build sandbox permits loopback (but not external) networking, so this stays hermetic — all bytes come from the local feed FODs, never the network. The server is killed on phase exit.
...
in pkgs.stdenv.mkDerivation {
  ...
  buildPhase = ''
    ...
    ${pkgs.python3}/bin/python -m http.server 8731 --directory ${feedFarm} & server=$!
