...
| `.` | T26 | Extend `test_download.sh`: for T25 — HTTP 429 + `Retry-After`, per-page retry, checkpoint resume, count guard, staging never in `public/` (§V.28–§V.32); for T28 — **120-day window cap**, **id-year bucketing** (CVE-2023→2023 feed regardless of pub date), `merge_window` upsert (newer wins / new insert), out-of-window prune, bootstrap (§V.35–§V.39) |
