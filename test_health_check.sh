...
test_health_check_passes() {
  ...
  (
    ...
    PATH="$mock_bin:$PATH" PAGES_URL="http://mock.test" \
...
MOCK
  ...
  (
    fix_mock_shebangs "$mock_bin"
    PATH="$mock_bin:$PATH" PAGES_URL="http://mock.test" MOCK_LOG="$log_file" \
      bash "$SCRIPT_DIR/health_check.sh"
  ) >/dev/null
...
MOCK
  ...
  (
    fix_mock_shebangs "$mock_bin"
    PATH="$mock_bin:$PATH" PAGES_URL="http://custom.pages.test/mirror" MOCK_LOG="$log_file" \
      bash "$SCRIPT_DIR/health_check.sh"
  ) >/dev/null
